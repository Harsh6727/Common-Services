...
import hospitalImage from './hosp.png';
...
const HospitalFinder: React.FC = () => {
    ...
    const [hospitals, setHospitals] = useState<{ name: string, address: string }[]>([]);
    const [page, setPage] = useState<number>(1);
    const [hasMore, setHasMore] = useState<boolean>(true);

    useEffect(() => {
        fetch('http://localhost:4000/api/healthcare/states')
    ...
    useEffect(() => {
        if (selectedState) {
            fetch(`http://localhost:4000/api/healthcare/cities?state=${selectedState}`)
                .then(response => response.json())
    ...
    const fetchHospitals = (state: string, city: string, page: number) => {
        fetch(`http://localhost:4000/api/healthcare/hospitals?state=${state}&city=${city}&page=${page}`)
            .then(response => response.json())
            .then(data => {
                setHospitals(data);
            ...
            <div className="main-content">
                <div className="left-section">
                    <div className="main-heading-content">
                        ...
                        <p>Enter your state and city to discover a list of hospitals located in your area.</p>
                    </div>
                    <div className="input-container">
                        ...
                        <div className="input-box">
                            ...
                            <p style={{ fontSize: '13px', opacity: 0.6 }}>List of Hospitals in Your City</p>
                            <button className="view-details-link" onClick={handleSearch}>Search</button>
                        </div>
                    </div>
                </div>
                <div className="right-section">
                    <img src={hospitalImage.src} alt="Hospital" />
                </div>
            </div>
            {hospitals.length > 0 && (
